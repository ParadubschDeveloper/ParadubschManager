...
public class BazaarSellPriceSelectorButton extends AbstractGuiItem {
    @Override
    public void onClick(Player p) {
        BazaarItemData data = (BazaarItemData) this.itemArgs.get(0);
        String key = "blank";
        if (data != null) key = data.getMaterial().toString();
        this.prompt(PromptType.INTEGER, key + "SellPrice");
    }
    ...
    public void build() {
        ...
        String prompt = this.getPrompt(key + "SellPrice");
        ...
        if (integer == null && !StringValidator.isEmpty(prompt)) {
            ...
            if (integer > data.getOffer()) {
                this.addLore(Message.Gui.INVALID_PRICE_LORE);
                this.addLore(Message.Gui.SELL_PRICE_TO_HIGH, data.getOffer() + "");
            } else if (integer <= data.getBuy()) {
                this.addLore(Message.Gui.INVALID_PRICE_LORE);
                this.addLore(Message.Gui.SELL_PRICE_TOO_LOW, data.getBuy() + "");
            } else {
                price = integer;
            }
        }
        ...
        this.getKvStore().set(key + "SellPrice", price);
    }
}
